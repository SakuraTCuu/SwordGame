import { Asset, assetManager, Component, Director, director, resources } from "cc";
...
export default class PromiseHelper {
    ...
    */
    public static wait<T extends Component>(delay: number, comp?: T) {
	 ...
	 * @param url
	 */
    public static loadRes<T extends Asset>(url: string, type: typeof Asset): Promise<T> {
        return new Promise((resolve, reject) => {
	 ...
	 * @param url
	 */
    public static loadResArray<T extends Asset>(
        url: string[],
        type: typeof Asset
    ): Promise<T[]> {
        return new Promise((resolve, reject) => {
    ...
    */
    public static loadRemote(url: string, opttion = {}): Promise<any> {
        return new Promise((resolve, reject) => {
            assetManager.loadRemote(url, opttion,
                (err: Error, res: any) => {
                    if (err) {
                        reject(err);
                    } else {
