...
export class weaponManager extends Component {
    ...
    loadPrefab(fileName, callback = null) {
        ...
        app.loader.load("resources", path, (err, assets) => {
            if (callback) {
                ...
                app.pool.plm.addPoolToPools(fileName, new NodePool(), assets);
            }
        });
    ...
    openSpell(total: number, radius: number = 180) {
        ...
        for (let i = 0; i < par.children.length; i++) {
            ...
            app.pool.plm.putToPool("spell", child);
            i--;
        };
        ...
        for (let i = 0; i < arr.length; i++) {
            let pos = arr[i];
            let spell = app.pool.plm.getFromPool("spell");
            if (spell) {
                this.initSpell(spell, pos, i * unitA);
            } else {
                this.loadPrefab("spell", (assets) => {
                    app.pool.plm.addPoolToPools("spell", new NodePool(), assets);
                    spell = app.pool.plm.getFromPool("spell");
                    this.initSpell(spell, pos, i * unitA);
                });
            }
        };
    ...
    usingSpellUltimateSkill() {
        ...
        for (let i = 0; i < arr.length; i++) {
            let pos = arr[i];
            let spell = app.pool.plm.getFromPool("spell");//能释放大招 spell 一定加载完毕 无需防护动态加载
            spell.getComponent("Spell").init(data, this._spellLv);
            spell.parent = par;
            spell.setPosition(pos[0], pos[1], 0);
            let angle = i * unitA;
    ...
    createSpiritBullet() {
        ...
        for (let i = 0; i < total; i++) {
            let spiritBullet = app.pool.plm.getFromPool("spiritBullet");
            let t = 1 / 10 * i;
            if (spiritBullet) {
                this.scheduleOnce(() => {
                ...
                this.loadPrefab("spiritBullet", (assets) => {
                    app.pool.plm.addPoolToPools("spiritBullet", new NodePool(), assets);
                    spiritBullet = app.pool.plm.getFromPool("spiritBullet");
                    this.scheduleOnce(() => {
                        this.initSpiritBullet(spiritBullet, initPosArr[i], flyDir);
                    }, t);
                });
    ...
    createSword() {
        ...
        let initPosArr = this.getFlySwordInitPos(total);
        for (let i = 0; i < total; i++) {
            let sword = app.pool.plm.getFromPool("sword");
            if (sword) this.initSword(sword, initPosArr[i]);
            else {
                this.loadPrefab("sword", (assets) => {
                    app.pool.plm.addPoolToPools("sword", new NodePool(), assets);
                    sword = app.pool.plm.getFromPool("sword");
        ...
        for (let i = 0; i < total; i++) {
            ...
            if (darts) this.initDarts(darts, flyDirArr[i], i);
                ...
                this.loadPrefab("darts", (assets) => {
                    app.pool.plm.addPoolToPools("darts", new NodePool(), assets);
                    darts = app.pool.plm.getFromPool("darts");
    ...
    createIceSpiritNeedle() {
        ...
        if (!flyDirs) return;
        for (let i = 0; i < total; i++) {
            let iceSpiritNeedle = app.pool.plm.getFromPool("iceSpiritNeedle");
            ...
            if (iceSpiritNeedle) {
                ...
                this.loadPrefab("iceSpiritNeedle", (assets) => {
                    app.pool.plm.addPoolToPools("iceSpiritNeedle", new NodePool(), assets);
                    iceSpiritNeedle = app.pool.plm.getFromPool("iceSpiritNeedle", true);
                    this.scheduleOnce(() => {
                        this.initIceSpiritNeedle(iceSpiritNeedle, flyDir);
                    }, t);
                });
    ...
    createSkyThunder() {
        ...
        for (let i = 0; i < total; i++) {
            ...
            this.scheduleOnce(() => {
                ...
                if (!wp) return;
                let skyThunder = app.pool.plm.getFromPool("skyThunder");
                if (skyThunder) {
                    this.initSkyThunder(skyThunder, wp);
                }
                else {
                    this.loadPrefab("skyThunder", (assets) => {
                        app.pool.plm.addPoolToPools("skyThunder", new NodePool(), assets);
                        // skyThunder = app.pool.plm.getFromPool("skyThunder");
                        skyThunder = app.pool.plm.getFromPool("skyThunder");
                        this.initSkyThunder(skyThunder, wp);
                    });
                }
    ...
    createLandFire() {
        ...
        for (let i = 0; i < total; i++) {
            this.scheduleOnce(() => {
                if (Constant.GlobalGameData.stopAll) return;
                let landFire = app.pool.plm.getFromPool("landFire");
                if (landFire) this.initLandFire(landFire, dirs[i % dirs.length]);
                else {
                    this.loadPrefab("landFire", (assets) => {
                        app.pool.plm.addPoolToPools("landFire", new NodePool(), assets);
                        landFire = app.pool.plm.getFromPool("landFire");
    ...
    createTaijihuan() {
        ...
        for (let i = 0; i < total; i++) {
            this.scheduleOnce(() => {
                if (Constant.GlobalGameData.stopAll) return;
                let taijihuan = app.pool.plm.getFromPool("taijihuan");
                if (taijihuan) this.initTaijihuan(taijihuan);
                else {
                    this.loadPrefab("taijihuan", (assets) => {
                        app.pool.plm.addPoolToPools("taijihuan", new NodePool(), assets);
                        taijihuan = app.pool.plm.getFromPool("taijihuan");
    ...
    usingSkillSwordRain() {
        ...
        this.schedule(() => {
            for (let i = 0; i < total; i++) {
                let swordRain = app.pool.plm.getFromPool("swordRain");
                if (swordRain) {
                    this.initSwordRain(swordRain, (spx + i * unitX));
                } else {
                    this.loadPrefab("swordRain", (assets) => {
                        app.pool.plm.addPoolToPools("swordRain", new NodePool(), assets);
                        swordRain = app.pool.plm.getFromPool("swordRain");
                        this.initSwordRain(swordRain, (spx + i * unitX));
                    });
                }
            }
    ...
    usingSkillJustOneSwordDivideWorld() {
        let prefab = app.pool.plm.getFromPool("justOneSwordDivideWorld");
        if (prefab) this.initJustOneSwordDivideWorld(prefab);
        else {
            this.loadPrefab("justOneSwordDivideWorld", (assets) => {
                app.pool.plm.addPoolToPools("justOneSwordDivideWorld", new NodePool(), assets);
                let prefab = app.pool.plm.getFromPool("justOneSwordDivideWorld");
    ...
    usingSkillThousandsSwordToTomb() {
        let total = 30;
        for (let i = 0; i < total; i++) {
            let prefab = app.pool.plm.getFromPool("thousandsSwordToTomb");
            if (prefab) {
                ...
                this.loadPrefab("thousandsSwordToTomb", (assets) => {
                    app.pool.plm.addPoolToPools("thousandsSwordToTomb", new NodePool(), assets);
                    let prefab = app.pool.plm.getFromPool("thousandsSwordToTomb");
    ...
    usingSkillIceCone() {
        ...
        let initPosArr = Utils.getCirclePos(r, total);
        for (let i = 0; i < total; i++) {
            let prefab = app.pool.plm.getFromPool("iceCone");
            let initPos = initPosArr[i];
            let dir = { x: initPos[0] / r, y: initPos[1] / r };
            if (prefab) {
                ...
                this.loadPrefab("iceCone", (assets) => {
                    app.pool.plm.addPoolToPools("iceCone", new NodePool(), assets);
                    let prefab = app.pool.plm.getFromPool("iceCone");
                    this.initIceCone(prefab, dir, initPos);
                });
            }
    ...
    usingSkillFireBloom() {
        let total = 60;
        for (let i = 0; i < total; i++) {
            let prefab = app.pool.plm.getFromPool("fireBloom");
            if (prefab) {
                ...
                this.loadPrefab("fireBloom", (assets) => {
                    app.pool.plm.addPoolToPools("fireBloom", new NodePool(), assets);
                    let prefab = app.pool.plm.getFromPool("fireBloom");
    ...
    usingSkillHellFire() {
        let total = 20;
        for (let i = 0; i < total; i++) {
            let prefab = app.pool.plm.getFromPool("hellFire");
            if (prefab) {
                ...
                this.loadPrefab("hellFire", (assets) => {
                    app.pool.plm.addPoolToPools("hellFire", new NodePool(), assets);
                    let prefab = app.pool.plm.getFromPool("hellFire");
    ...
    createMoveLikeFire() {
        if (Constant.GlobalGameData.stopAll) return;
        let prefab = app.pool.plm.getFromPool("moveLikeFire");
        if (prefab) {
            let wp = em.dispatch("getHeroWorldPos");
            let layer = find("Canvas/bulletLayer");
            prefab.parent = layer;
            ...
            this.loadPrefab("moveLikeFire", (assets) => {
                app.pool.plm.addPoolToPools("moveLikeFire", new NodePool(), assets);
                let prefab = app.pool.plm.getFromPool("moveLikeFire");
                let wp = em.dispatch("getHeroWorldPos");
                let layer = find("Canvas/bulletLayer");
                prefab.parent = layer;
    ...
    usingSkillDangerWindToNear() {
        ...
        for (const dir of dirArr) {
            let prefab = app.pool.plm.getFromPool("dangerWindToNear");
            if (prefab) {
                ...
                this.loadPrefab("dangerWindToNear", (assets) => {
                    app.pool.plm.addPoolToPools("dangerWindToNear", new NodePool(), assets);
                    let prefab = app.pool.plm.getFromPool("dangerWindToNear");
                    prefab.parent = find("Canvas/bulletLayer");
                    prefab.setWorldPosition(wp.x, wp.y, 0);
                    prefab.getComponent("DangerWindToNear").init(dir);
    ...
    usingSkillDoomsdayStorm() {
        ...
        while (total) {
            let prefab = app.pool.plm.getFromPool("doomsdayStorm");
            if (prefab) queue.enqueue(prefab);
            else {
                this.loadPrefab("doomsdayStorm", (assets) => {
                    app.pool.plm.addPoolToPools("doomsdayStorm", new NodePool(), assets);
                    let prefab = app.pool.plm.getFromPool("doomsdayStorm");
    ...
    usingSkillThunderRunning() {
        ...
        for (let i = 0; i < total; i++) {
            ...
            let prefab = app.pool.plm.getFromPool("thunderRunning");
            if (prefab) {
                queue.enqueue(prefab);
            } else {
                this.loadPrefab("thunderRunning", (assets) => {
                    app.pool.plm.addPoolToPools("thunderRunning", new NodePool(), assets);
                    let prefab = app.pool.plm.getFromPool("thunderRunning");
                    queue.enqueue(prefab);
                });
            }
    ...
    usingSkillThunderFissionBead() {
        let flyDir = this.getFlyDir(em.dispatch("getHeroControlProperty", "_curAimDir"));
        let prefab = app.pool.plm.getFromPool("thunderFissionBead");
        ...
        if (prefab) {
            ...
            this.loadPrefab("thunderFissionBead", (assets) => {
                app.pool.plm.addPoolToPools("thunderFissionBead", new NodePool(), assets);
                let prefab = app.pool.plm.getFromPool("thunderFissionBead");
                prefab.parent = find("Canvas/bulletLayer");
                let wp = em.dispatch("getHeroWorldPos");
                prefab.setWorldPosition(wp);
    ...
    */
    ...
    createLongSword() {
        ...
        for (let i = 0; i < total; i++) {
            let sword = app.pool.plm.getFromPool("longSword");
            if (sword) this.initLongSword(sword, flyDirs[i]);
            else {
                this.loadPrefab("longSword", (assets) => {
                    app.pool.plm.addPoolToPools("longSword", new NodePool(), assets);
                    sword = app.pool.plm.getFromPool("longSword");
    ...
    createGiantSword() {
        if (Constant.GlobalGameData.stopAll) return;
        let giantSword = app.pool.plm.getFromPool("giantSword");
        if (giantSword) this.initGiantSword(giantSword);
        else {
            this.loadPrefab("giantSword", (assets) => {
                app.pool.plm.addPoolToPools("giantSword", new NodePool(), assets);
                giantSword = app.pool.plm.getFromPool("giantSword");
    ...
    createGiantAxe() {
        if (Constant.GlobalGameData.stopAll) return;
        let giantAxe = app.pool.plm.getFromPool("giantAxe");
        if (giantAxe) this.initGiantAxe(giantAxe);
        else {
            this.loadPrefab("giantAxe", (assets) => {
                app.pool.plm.addPoolToPools("giantAxe", new NodePool(), assets);
                giantAxe = app.pool.plm.getFromPool("giantAxe");
    ...
    createRotationAxe() {
        if (Constant.GlobalGameData.stopAll) return;
        let rotationAxe = app.pool.plm.getFromPool("rotationAxe");
        if (rotationAxe) this.initRotationAxe(rotationAxe);
        else {
            this.loadPrefab("rotationAxe", (assets) => {
                app.pool.plm.addPoolToPools("rotationAxe", new NodePool(), assets);
                rotationAxe = app.pool.plm.getFromPool("rotationAxe");
    ...
    createGiantArrow() {
        if (Constant.GlobalGameData.stopAll) return;
        let giantArrow = app.pool.plm.getFromPool("giantArrow");
        if (giantArrow) this.initGiantArrow(giantArrow);
        else {
            this.loadPrefab("giantArrow", (assets) => {
                app.pool.plm.addPoolToPools("giantArrow", new NodePool(), assets);
                giantArrow = app.pool.plm.getFromPool("giantArrow");
    ...
    createContinueArrow() {
        if (Constant.GlobalGameData.stopAll) return;
        let continueArrow = app.pool.plm.getFromPool("continueArrow");
        if (continueArrow) this.initContinueArrow(continueArrow);
        else {
            this.loadPrefab("continueArrow", (assets) => {
                app.pool.plm.addPoolToPools("continueArrow", new NodePool(), assets);
                continueArrow = app.pool.plm.getFromPool("continueArrow");
    ...
    createShockWave(wp, flyDir) {
        let shockWave = app.pool.plm.getFromPool("shockWave");
        if (shockWave) this.initShockWave(shockWave, wp, flyDir);
        else {
            this.loadPrefab("shockWave", (assets) => {
                app.pool.plm.addPoolToPools("shockWave", new NodePool(), assets);
                shockWave = app.pool.plm.getFromPool("shockWave");
